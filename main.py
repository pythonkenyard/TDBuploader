...
def initial_setup(cfg):
    ...
    if any (word in trackername.lower() for word in tdb.uploadtrackers):
        ...
        if autoupload.lower() == "y":
            ...
            profile = 5
            isExist = False
            print("Checking possible Chrome settings for autoupload:")
            while isExist == False:
            ...
            print(f"Selected Chrome profile {setprofile}")

            cwd = os.getcwd()
            download_directory = f"{cwd}/downloads"
            print(f"Uploaded torrents will be saved to {download_directory}\n")
            ...
            if qbit_check:
                ...
                print(f"Qbittorrent location found to be {qbit_loc}")
            else:
